...
const actions = {
    ...
    async addTodo({ commit }, title) {
        const response_add = await axios.post(`https://jsonplaceholder.typicode.com/todos`, { title: title, completed: false })

        commit('addTodo', response_add.data)
    },
...
const mutations = {
    ...
    updateTodo: (state, r_updateVaule) => {
        const index = state.todos.findIndex(todo => todo.id === r_updateVaule.id)
        if (index !== -1)
            state.todos.splice(index, 1, r_updateVaule);
    },
